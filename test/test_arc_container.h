...
template<
  class Forest_options,
  template< class > class Node_container_template
>
class Arc_container : protected Node_container_template< Forest_options > {
 ...
 public:
  ...
  void insert_arc(const Arc& arc) {
    ...
    bool cancel_below = resolve_overlaps_before(arc_it);
    if (!cancel_below) {
      resolve_overlaps_after(arc_it);
    }
  }
  ...
  template<
    class Tag,
    class Iterator = typename Arc_multi_index_container::template index< Tag >::type::iterator
  >
   ...
   */
  Arc_iterator resolve_overlaps_after(Arc_iterator lower_arc_it) {
   ...
   */
  bool resolve_overlaps_before(Arc_iterator upper_arc_it) {
  ...
  /* Raise all arcs below the given node.
   */
  template< class Tag >
